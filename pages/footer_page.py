...
class FooterPage(BasePage):
    ...
    def checking_not_contents_page(self, contents):
        key = True
        for element in contents:
    ...
    def click_button_create_landing(self, locator_btn, locator_rsl, rsl):
        button = self.search_element(locator_btn)
        button.click()
        time.sleep(2)
        next_wnd = self.driver.window_handles[1]
        self.driver.switch_to.window(next_wnd)
        text = self.search_element(locator_rsl).text
        assert text == rsl
